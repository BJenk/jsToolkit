...
const  lowestPosNotInArr = (arr) => {
    let posArr =  Array(...new Set(arr)) // Use Set() to remove duplicate integers
        .filter(x => x>0)                // Remove N<1
        .map(x => Number(x))             // Convert everything to number (this was to make intellisense happy)
        .sort()                          // Sort Low-High
    let count = posArr[0]-1
    for(let i = 0; i<posArr.length; i++){
        if(posArr[i] - 1 != count){
            return posArr[i] - 1
        }else if(posArr[i+1] === undefined){ //If the entire array is sequential
            return posArr[i] + 1
        }
        else{
            count = count + 1
